...
import TreeMap from './components/TreeMap.vue'; // <--- Importiamo il componente Mappa
...
// --- 1. CHIAMATE API (Caricamento Dati) ---
...
// --- 2. AZIONE UTENTE (Innaffia) ---
// Questa funzione viene chiamata sia dal bottone della Card sia dal Popup della Mappa
const waterTree = (treeId) => {
  if (!currentUser.value) return alert("Utente non caricato!");
  
  // Inviamo al server chi ha innaffiato (userId) e quale albero (treeId)
...
onMounted(() => {
  ...
  // A. Aggiornamento Alberi (Real-time per tutti)
  ...
  // B. Aggiornamento Utente (Solo per me: XP e Livello)
  ...
  // C. Animazione Level Up
  socket.on('level_up', () => {
    showLevelUp.value = true;
    setTimeout(() => showLevelUp.value = false, 3000); // Nascondi dopo 3 sec
  });
});
</script>
...
.separator { text-align: center; margin: 20px 0; color: #95a5a6; font-size: 0.9rem; text-transform: uppercase; letter-spacing: 2px; }
...
.level-up-modal {
  ...
  z-index: 999; text-shadow: 1px 1px 2px rgba(0,0,0,0.1);
}
