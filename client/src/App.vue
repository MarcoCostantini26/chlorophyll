...
onMounted(() => {
  ...
  socket.on('level_up', (d) => { showLevelUp.value = true; setTimeout(() => showLevelUp.value = false, 3000); });
  socket.on('badge_unlocked', (d) => { 
    if (currentUser.value && currentUser.value.username === d.username) { 
      lastUnlockedBadge.value = d.badge; showBadgeModal.value = true; setTimeout(() => showBadgeModal.value = false, 4000); 
    } 
  });
});
</script>
...
.app-header {
  ...
  width: 100%;
  height: 70px;
  background: #1e1e1e; /* HEADER SCURO */
...
/* PADDING PER NON FINIRE SOTTO L'HEADER */
.main-content {
  padding-top: 90px; 
  padding-bottom: 40px;
  min-height: 100vh;
...
.main-title { color: #2ecc71; font-size: 1.5rem; margin: 0; font-weight: 800; } /* Verde più acceso */
...
.nav-item { text-decoration: none; color: #bdc3c7; /* Testo grigio chiaro */ font-weight: 600; font-size: 0.95rem; transition: color 0.2s; background: none; border: none; font-family: inherit; cursor: pointer; display: flex; align-items: center; height: 70px; border-bottom: 3px solid transparent; }
...
/* MODALI STILI (Adattati leggermente) */
.ai-modal-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.7); z-index: 2000; display: flex; justify-content: center; align-items: center; padding: 20px; backdrop-filter: blur(2px); }
...
.ai-body { padding: 25px; color: #ecf0f1; line-height: 1.6; font-size: 1.05rem; }
...
/* Level Up & Badge */
.level-up-modal { position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: #f1c40f; color: white; padding: 20px 40px; border-radius: 50px; font-size: 2rem; z-index: 3000; font-weight: 900; box-shadow: 0 10px 30px rgba(241, 196, 15, 0.6); animation: popIn 0.5s; text-shadow: 0 2px 5px rgba(0,0,0,0.2); }
.badge-modal { position: fixed; top: 30%; left: 50%; transform: translate(-50%, -50%); background: #2c3e50; border: 4px solid #f1c40f; color: white; padding: 30px; text-align: center; border-radius: 20px; box-shadow: 0 20px 60px rgba(0,0,0,0.5); z-index: 4000; animation: popIn 0.5s; min-width: 300px; }
