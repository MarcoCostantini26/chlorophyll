...
const computedStats = computed(() => {
  ...
  all.forEach(t => {
    ...
    if (!cats[c]) cats[c] = { healthy:0, thirsty:0, critical:0 };
    cats[c][t.status]++;
  });
...
const statusChartData = computed(() => ({ 
  ...
  datasets: [{ 
    data: [computedStats.value.healthy, computedStats.value.thirsty, computedStats.value.critical], 
...
const categoryChartData = computed(() => { 
  ...
  return { 
    labels, 
    datasets: [ 
      { label: 'Sani', data: Object.values(cats).map(c => c.healthy), backgroundColor: '#2ecc71' }, 
      { label: 'Assetati', data: Object.values(cats).map(c => c.thirsty), backgroundColor: '#f1c40f' }, 
      { label: 'Critici', data: Object.values(cats).map(c => c.critical), backgroundColor: '#e74c3c' } 
    ] 
  }; 
});
...
const filteredTrees = computed(() => {
  let result = treeStore.trees.filter(t => {
    ...
    const tCity = t.city || 'N/D';
    
    const matchesSearch = t.name.toLowerCase().includes(term) || tCity.toLowerCase().includes(term);
    const matchesCategory = activeFilters.value.category ? (t.category || 'other') === activeFilters.value.category : true;
    const matchesCity = activeFilters.value.city ? tCity === activeFilters.value.city : true;
  ...
  <div class="analytics-page">
    ...
    <div class="content-stack">
      ...
      <div class="kpi-scroller">
        <div class="kpi-card total"><span class="val">{{ computedStats.total }}</span><span class="lbl">Totale</span></div>
        <div class="kpi-card healthy"><span class="val">{{ computedStats.healthy }}</span><span class="lbl">Sani</span></div>
        <div class="kpi-card thirsty"><span class="val">{{ computedStats.thirsty }}</span><span class="lbl">Assetati</span></div>
        <div class="kpi-card critical"><span class="val">{{ computedStats.critical }}</span><span class="lbl">Critici</span></div>
      </div>
      ...
      <div class="charts-grid">
        <div class="card chart-card">
          ...
          <div class="chart-box">
             <Doughnut :data="statusChartData" :options="{ responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right', labels: { color: '#fff' } } } }" />
          </div>
        </div>
      ...
      <div class="card list-card">
        ...
        <div class="desktop-only table-container">
          <table class="admin-table">
            <thead>
              <tr>
                <th class="col-name sortable" @click="sortBy('name')">Nome <span v-if="sortKey==='name'">{{sortOrder===1?'🔼':'🔽'}}</span></th>
                ...
                <th class="col-status sortable" @click="sortBy('status')">Stato <span v-if="sortKey==='status'">{{sortOrder===1?'🔼':'🔽'}}</span></th>
                <th class="col-water sortable" @click="sortBy('waterLevel')">H2O <span v-if="sortKey==='waterLevel'">{{sortOrder===1?'🔼':'🔽'}}</span></th>
                <th class="col-trend">Trend</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="tree in filteredTrees" :key="tree._id" class="table-row">
                <td class="name-cell" @click.stop="goToMap(tree._id)"><strong>{{ tree.name }}</strong></td>
                <td><span class="badge-cat">{{ tree.category || 'N/D' }}</span></td>
                <td>{{ tree.city || 'N/D' }}</td>
